...
SELECT symbol, date, expiry, strike, option_type, time, open, high, low, close, volume, oi, option_nm, COUNT(*)
FROM public."Nifty_options"
GROUP BY symbol, date, expiry, strike, option_type, time, open, high, low, close, volume, oi, option_nm
HAVING COUNT(*) > 1;
----------------------------------------------------------------------------------------------------
...
VALUES (15,5,'full_candle_breakout','full_candle_breakout', 60, 80, 50, 4, 1,20,'15:20:00',1,75,50,3,85,2,2,900000,3,'regular_system_sl',20,50)
...
DROP MATERIALIZED VIEW IF EXISTS mv_ranked_breakouts_with_rounds_for_reentry;
...
hedge_base_strike_selection AS ( 
    SELECT 
		...
		CASE WHEN o.strike = b.atm_strike THEN 1 ELSE 2 END AS atm_priority,
        ROW_NUMBER() OVER (
            PARTITION BY b.trade_date, b.expiry_date 
            ORDER BY 
                -- CASE WHEN o.strike = b.atm_strike THEN 1 ELSE 2 END,
...
atm_candidate AS (
    SELECT h.*
    FROM hedge_base_strike_selection h
	JOIN strategy s ON TRUE
    WHERE atm_priority = 1
      AND hedge_price <= s.hedge_entry_price_cap
    --LIMIT 1
),
final_selection AS (
    SELECT * FROM atm_candidate
    UNION ALL
    SELECT *
    FROM hedge_base_strike_selection
    WHERE rn = 1
       AND 
       NOT (hedge_base_strike_selection.trade_date IN (SELECT trade_date FROM atm_candidate))
),
...
nifty_range AS (
    SELECT 
        ...
        ha_high AS breakout_high,
        ha_low AS breakout_low
    FROM mv_ha_big_candle  -- assumed to contain 9:15 breakout range
)
    SELECT 
        l.*,
        o.time AS ltp_time,
